...
*  原生sql查询语句
 */
func SqlRows(sql string, fields []string) ([]map[string]string, error) {
	records := []map[string]string{}

	u_rows, err := gdb.DB.Raw(sql).Rows()
	if err != nil {
		return records, err
	}
	records, err = rows2Map(fields, u_rows)
...
*  原生sql查询语句
 */
func SqlRow(sql string, fields []string) (map[string]string, error) {
	u_row := gdb.DB.Raw(sql).Row()
	records, err := row2Map(fields, u_row)
...
把where条件 转成 字符串
*/
func WhereMap2Str(w map[string]string) string {
...
查询多行
*/
func Rows(table_name, order_by string, page_size, page int, fields []string, query interface{}, args ...interface{}) ([]map[string]string, error) {
	records := []map[string]string{}
	...
	u_rows, err := gdb.DB.Table(table_name).Select(fields).Where(query, args...).Limit(page_size).Order(order_by).Offset(offset).Rows()
	if err == nil {
		records, err = rows2Map(fields, u_rows)
	}
...
func Count(table_name string, query interface{}, args ...interface{}) int {

	var count int
	gdb.DB.Table(table_name).Where(query, args...).Count(&count)
