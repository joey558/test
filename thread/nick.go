...
// 女名
func FemaleNameItems() []string {
	str := "嘉,琼,桂,娣,叶,璧,璐,娅,琦,晶,妍,茜,秋,珊,莎,锦,黛,青,倩,婷,姣,婉,娴,瑾,颖,露,瑶,怡,婵,雁,蓓,纨,仪,荷,丹,蓉,眉,君,琴,蕊,薇,菁,梦,岚,苑,婕,馨,瑗,琰,韵,融,园,艺,咏,卿,聪,澜,纯,毓,悦,昭,冰,爽,琬,茗,羽,希,宁,欣,飘,育,滢,馥,筠,柔,竹,霭,凝,晓,欢,霄,枫,芸,菲,寒,伊,亚,宜,可,姬,舒,影,荔,枝,思,丽,秀,娟,英,华,慧,巧,美,娜,静,淑,惠,珠,翠,雅,芝,玉,萍,红,娥,玲,芬,芳,燕,彩,春,菊,勤,珍,贞,莉,兰,凤,洁,梅,琳,素,云,莲,真,环,雪,荣,爱,妹,霞,香,月,莺,媛,艳,瑞,凡,佳"
	str_arr := strings.Split(str, ",")
	return str_arr
}

// 男名
func MaleNameItems() []string {
	str := "涛,昌,进,林,有,坚,和,彪,博,诚,先,敬,震,振,壮,会,群,豪,心,邦,承,乐,绍,功,松,善,厚,庆,磊,民,友,裕,河,哲,江,超,浩,亮,政,谦,亨,奇,固,之,轮,翰,朗,伯,宏,言,若,鸣,朋,斌,梁,栋,维,启,克,伦,翔,旭,鹏,泽,晨,辰,士,以,建,家,致,树,炎,德,行,时,泰,盛,雄,琛,钧,冠,策,腾,伟,刚,勇,毅,俊,峰,强,军,平,保,东,文,辉,力,明,永,健,世,广,志,义,兴,良,海,山,仁,波,宁,贵,福,生,龙,元,全,国,胜,学,祥,才,发,成,康,星,光,天,达,安,岩,中,茂,武,新,利,清,飞,彬,富,顺,信,子,杰,楠,榕,风,航,弘"
	str_arr := strings.Split(str, ",")
	return str_arr
}

// 单姓
func FamilyNameItemsSin() []string {
...
// 复姓
func FamilyNameItemsSur() []string {
	str := "辕轩,令狐,钟离,宇文,长孙,幕容,鲜于,闾丘,司徒,司空,丌官,司寇,仉督,子车,颛孙,端木,巫马,公西,漆雕,乐正,壤驷,公良,拓拔,夹谷,宰父,谷梁,晋楚,阎法,汝鄢,涂钦,段干,百里,东郭,南门,呼延,归海,羊舌,微生,岳帅,缑亢,况后,有琴,梁丘,左丘,东门,西门,商牟,佘佴,佰赏,南官,墨哈,谯笪,年爱,阳佟,第五,言福"
	str_arr := strings.Split(str, ",")
	return str_arr
}

// 获取随机数字字符串
func GetRmInt() string {
...
// 从数组中取出一个字符串
func ArrFromOne(name_arr []string) string {

	r := rand.New(rand.NewSource(time.Now().UnixNano()))
	index := r.Intn(len(name_arr))

	if len(name_arr[index]) > 0 {
...
func GetNickName() string {
	nick_header := ArrFromOne(NickHeader())
	nick_foot := ArrFromOne(NickFoot())
	return nick_header + nick_foot
}

// 生成一个昵称带数字
func GetNickNameInt() string {
	return GetNickName() + GetRmInt()
}
...
func GetLastName() string {
	...
	x := r.Intn(2)
	if x == 0 {
		return ArrFromOne(FemaleNameItems())
	}

	return ArrFromOne(MaleNameItems())
}
...
func GetFamilyName() string {
	...
	x := r.Intn(2)
	if x == 0 {
		return ArrFromOne(FamilyNameItemsSin())
	}

	return ArrFromOne(FamilyNameItemsSur())
}

// 随机一个姓名(男|女)
func GetName() string {
	return GetLastName() + GetFamilyName()
}

// 随机一个姓名(男|女)
func GetNameInt() string {
	return GetName() + GetRmInt()
}

// 生成一个昵称1/2几率带数字
func GetNameAll() string {
	r := rand.New(rand.NewSource(time.Now().UnixNano()))
	x := r.Intn(4)
	if x == 0 {
		return GetNickNameInt()
	}
